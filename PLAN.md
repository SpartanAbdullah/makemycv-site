...
- **B3** 🔜 — Expand `/resume-checker` FAQ from 6 → 8–10 real Q&As (add: "what is an ATS", "how does the checker work", "why do ATS systems score differently", "PDF or DOCX in the UAE", "is ATS enough on its own", "keyword optimization without stuffing"). Keep SSR-present; FAQPage mirrors verbatim. *Acceptance: 8–10 answers in `view-source`; FAQPage valid; gates green.*
- **B1** 🔜 — Dense, always-visible "How the ATS checker works" section on `/resume-checker`: name real systems (Workday, Greenhouse, Lever, Taleo, iCIMS) + how each parses; enumerate the "60+ UAE-tuned checks" by category (content / structure / ATS-compat / design / UAE-specific); DO/DON'T list. Factual, no hype. *Acceptance: methodology sentence + system names in server HTML; gates green.*
